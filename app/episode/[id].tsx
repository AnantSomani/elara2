...
import { useSupermemoryChat } from '../../hooks/useSupermemoryChat';
...
export default function EpisodePage() {
  ...
  const {
    ...
    setScope,
    getSessionStats,
  } = useSupermemoryChat(episodeId || currentEpisode?.id || '');
  ...
  const getCurrentSpeaker = () => {
    const lastResponse = messages.slice().reverse().find(msg => msg.type === 'response');
    if (lastResponse) {
      // For now, return a default host name
      return 'Host';
    }
  ...
  // Defensive default for memorySearchStats
  const safeMemorySearchStats = { totalSearches: 0, memorySearches: 0, fallbackSearches: 0, averageMemoryResults: 0 };
       ...
       <View style={styles.bottomSheetContainer}>
         ...
         {isBottomSheetExpanded && (
           ...
           >
             ...
             {safeMemorySearchStats.totalSearches > 0 && (
               <View style={styles.memoryStatsContainer}>
                 <MemoryStats 
                   totalSearches={safeMemorySearchStats.totalSearches}
                   memorySearches={safeMemorySearchStats.memorySearches}
                   fallbackSearches={safeMemorySearchStats.fallbackSearches}
                   averageMemoryResults={safeMemorySearchStats.averageMemoryResults}
                   showDetails={true}
                 />
               </View>
             )}
