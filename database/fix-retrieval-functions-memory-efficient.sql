-- Memory-efficient retrieval functions for Supabase free tier
-- This version avoids creating large indexes that require too much memory
...
-- Create the main search function for semantic search (memory efficient)
...
-- Create a more flexible search function (memory efficient)
...
-- Create basic indexes (avoiding memory-intensive vector indexes)
-- These are much more memory efficient
CREATE INDEX IF NOT EXISTS idx_transcript_segments_episode_id 
ON transcript_segments(episode_id);

CREATE INDEX IF NOT EXISTS idx_transcript_segments_has_embedding 
ON transcript_segments(episode_id) 
WHERE embedding IS NOT NULL;

-- Add comments for documentation
COMMENT ON FUNCTION search_segments IS 'Search transcript segments by vector similarity for a specific episode (memory efficient)';
COMMENT ON FUNCTION match_segments IS 'Search transcript segments by vector similarity across all episodes or filtered by episode (memory efficient)';
